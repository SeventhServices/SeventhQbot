...
using NetworkModelExtension = Seventh.Bot.Extensions.NetworkModelExtension;
using ObjectExtension = Seventh.Bot.Extensions.ObjectExtension;
    ...
    public class MessageParser
    {
        ...
        private void AddClientParse()
        {
            _commandParser.Add(new StringParsing(c =>
            {
                ...
                if (NetworkModelExtension.CheckError(myPageResult, c))
                {
                    return;
                }
                ...
                c.ReturnMessage.Add($"[Version]:{RequestParams.Version}\n" +
                                    ...
                                    ObjectExtension.FormatToString(robotStatus.DownloadConfig));
            ...
            _commandParser.Add(new StringParsing((c, m, q) =>
            {
                ...
                if (NetworkModelExtension.CheckError(loginResult, c))
                {
                    return;
                }
                ...
                c.ReturnMessage.Add($"[{q}:签到成功" +
                                    ...
                                    NetworkModelExtension.FormatLoginBonus(myPageResult.MyPage.LoginBonusList));
            ...
            _commandParser.Add(new StringParsing((c, m, q) =>
            {
                ...
                if (NetworkModelExtension.CheckError(result, c))
                {
                    return;
                }
                ...
                c.ReturnMessage.Add($"[{q}:Presents" +
                                    ...
                                    string.Join("\n", result.PresentBox.UserPresentList.Take(3).Select(p => ObjectExtension.FormatToString(p))));
        ...
        private void AddSetParse()
        {
            _commandParser.Add(new StringParsing((c, m) =>
                {
                    c.ReturnMessage.Add(ObjectExtension.FormatToString(typeof(RequestParams)));
                })
                .WhenStartWith("当前请求参数"));

            _commandParser.Add(new StringParsing((c, m) =>
                {
                    c.ReturnMessage.Add(ObjectExtension.FormatToString(ConfigureWatcher.GetFreshConfigure<RobotStatus>()));
                })
                .WhenStartWith("当前状态参数"));

            _commandParser.Add(new StringParsing((c, m) =>
                {
                    c.ReturnMessage.Add(ObjectExtension.FormatToString(typeof(MemoryCache)));
                })
        ...
        private void AddBindParse()
        {
            ...
            _commandParser.Add(new StringParsing((c, m, q) =>
            {
                ...
                c.ReturnMessage.Add($"{pidIndex}/{idIndex} /n" +
                    $"{m}");
                try
                {
                    ...
                    var binding = new AccountBinding
                    {
                        Qq = q,
                        BindTime = DateTime.Now,
                    ...
                    binding.BoundAccountPid = binding.BoundAccount.Pid;

                    var addResult = _bindRepository.Add(binding);
                    if (addResult == null)
                    {
                        return;
                    }
                    ...
                    c.ReturnMessage.Add($"[pid];{result.Pid}\n" +
                                        ...
                                        $"{ObjectExtension.FormatToString(result.Inspection.DownloadConfig)}");
                }
                catch (DbException e)
                {
            ...
            _commandParser.Add(new StringParsing((c, m) =>
           {
            ...
            _commandParser.Add(new StringParsing(((c, m) =>
            {
                ...
                if (paramExisted)
                {
                    ...
                    c.ReturnMessage.Add(NetworkModelExtension.FormatRanking(result, DateTime.Now, 7));
                    return;
                }
                ...
                else
                {
                    ...
                    MemoryCache.EventRankingCache = NetworkModelExtension.FormatBorder(result, robotStatus.LastEventBorderDateTime);
                    ConfigureWatcher.RefreshConfigure<RobotStatus>(robotStatus);
                    c.ReturnMessage.Add(MemoryCache.EventRankingCache);
                }
            })).WhenStartWith("档线"));
            ...
            _commandParser.Add(new StringParsing(((c, m) =>
            {
                ...
                if (paramExisted)
                {
                    ...
                    if (NetworkModelExtension.CheckError(maxRankResult, c))
                    {
                        return;
                    }

                    c.ReturnMessage.Add(NetworkModelExtension.FormatRanking(maxRankResult, DateTime.Now, 7));
                    return;
                }

                var robotStatus = ConfigureWatcher.GetFreshConfigure<RobotStatus>();
                var result = _apiClient.EventRankingUser(new EventRankingUserRequest
                { MusicId = 83 , EventType = robotStatus.OpenEventType, RankingType = RankingCategory.HighScoreRanking }).GetAwaiter().GetResult();
                ...
                c.ReturnMessage.Add(NetworkModelExtension.FormatBorder(result, DateTime.Now));
            ...
            _commandParser.Add(new StringParsing(((c, m, q) =>
            {
                ...
                if (bind != null)
                {
                    ...
                    c.ReturnMessage.Add(NetworkModelExtension.FormatRankingWithPickupPid(result, DateTime.Now, pid, 7)
